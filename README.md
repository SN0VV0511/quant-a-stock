...
启用 `BACKTEST_AUTO_GENERATE=true` 后，如果 `reports/backtest_latest.json` 缺失或过期，
Web 会调用 `scripts.robust_walk_forward`；失败后等待 15 分钟再自动重试。
4 核 4G 的 Docker 配置默认关闭自动回测，已有报告仍正常展示。
历史股票池不足 42 个月、任一历史池成员缺少行情、基准缺少回测区间端点时明确报错，
不再展示旧策略或用当前股票池回填历史。手动刷新命令：
...
默认编排按 4 核 4G 配置：守护进程最多 2.5 CPU / 2304 MiB，面板最多
1 CPU / 768 MiB，常驻容器内存上限合计 3 GiB，给宿主机留约 1 GiB。
BaoStock 历史任务默认 2 路并发，数值库每进程 1 个计算线程。
这是资源预算，尚未在目标服务器做峰值压力测试；手动预览扫描也计入面板限额。
完整多参数回测建议在独立研究环境运行，查看面板不会默认启动它。
...
`benchmark/000905.csv` 可选。股票池版本不足 42 个月或历史成员行情不完整时脚本会拒绝运行：
...
页面以 Three.js 三维办公室作为主界面：五位同事分别负责行情、策略、持仓、执行
和巡检，几何建模的工位、行情看板、茶水间与休息区可拖动旋转、滚轮缩放。
人物会工作、走动、拿杯子休息和交流；点击人物会拉近镜头、转身招呼、亮起光环，
并在同一场景内打开真实数据面板。手机使用底部面板，保留上方的三维人物。

场景生活与交易任务独立，点击人物只导航；看板柱形取自已有账户净值，不生成行情。
渲染在浏览器执行，默认最多 30 帧/秒、DPR 上限 1.5；低耗模式关闭阴影、最多
20 帧/秒且 DPR 为 1。可暂停人物生活，后台标签页暂停绘制和轮询，系统减弱动态
设置同样生效。需要支持 WebGL 2 的浏览器；无法启动三维场景时仍可打开全部面板。
累计收益、当前持仓与真实健康状态分别展示；行情不可用时保留账本估值并显示降级
标记，不把缺失报价当作零元。
