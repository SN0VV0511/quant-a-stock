...
ROBUST_V2_ETF_TARGET = float(os.getenv("ROBUST_V2_ETF_TARGET", "0.48"))
ROBUST_V2_STOCK_TARGET = float(os.getenv("ROBUST_V2_STOCK_TARGET", "0.50"))
ROBUST_V2_MIN_CASH = float(os.getenv("ROBUST_V2_MIN_CASH", "0.20"))
ROBUST_V2_MAX_SINGLE_ETF = float(os.getenv("ROBUST_V2_MAX_SINGLE_ETF", "0.24"))
ROBUST_V2_MAX_SINGLE_STOCK = float(os.getenv("ROBUST_V2_MAX_SINGLE_STOCK", "0.16"))
# 5 万账户的 16% 单票预算为 8,000 元；整手取整后通常低于该值，因此 V2
# 使用独立的 4,000 元门槛，不改变旧策略的 8,000 元全局默认值。
ROBUST_V2_MIN_STOCK_ORDER_AMOUNT = float(
    os.getenv("ROBUST_V2_MIN_STOCK_ORDER_AMOUNT", "4000.0")
)
# robust_v2 只用宽基 ETF 承担市场 Beta，行业 ETF 仍可供旧 RPS 策略观察。
ROBUST_V2_BROAD_ETF_CODES: frozenset[str] = frozenset(
    {"510300", "510500", "512100", "159915", "588000"}
)
ROBUST_V2_REBALANCE_DAYS = int(os.getenv("ROBUST_V2_REBALANCE_DAYS", "20"))
...
ROBUST_V2_SIGNAL_STRUCTURAL_MAX_ATTEMPTS = int(
    os.getenv("ROBUST_V2_SIGNAL_STRUCTURAL_MAX_ATTEMPTS", "3")
)
ROBUST_V2_DAILY_JOB_RETRY_SECONDS = int(
    os.getenv("ROBUST_V2_DAILY_JOB_RETRY_SECONDS", "900")
)
...
TRAILING_GIVEBACK_RSI75 = 0.30
TRAILING_GIVEBACK_RSI80 = 0.25
LIMITUP_INTRADAY_DRAWDOWN = 0.04
LIMITUP_VWAP_BREAK_MINUTES = 3
...
# 腾讯接口行业指数代码映射（AKShare 不稳定时的兜底）
ENABLE_TENCENT_INDUSTRY_FALLBACK = True
INDUSTRY_TENCENT_MAP: dict[str, str] = {
    "证券": "sz399975",
    "半导体": "cs990001",
    "光伏设备": "cs931151",
    "电池": "cs931719",
...
# 开盘急速止盈: 开盘后 N 分钟内，持仓盈利超过阈值则立即卖出
OPENING_TAKEPROFIT_MINUTES = int(os.getenv("OPENING_TAKEPROFIT_MINUTES", "15"))
